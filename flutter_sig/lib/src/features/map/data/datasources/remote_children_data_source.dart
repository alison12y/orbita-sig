...
class RemoteChildrenDataSourceImpl implements RemoteChildrenDataSource {
  // Use env var or fallback to Render backend
  static String get _baseUrl =>
      dotenv.env['API_URL'] ?? 'https://backend-sig-l0wo.onrender.com';
  ...
  Future<String> regenerateCode(String childId) async {
    ...
    // Fallback para modo demo/local sin backend real
    // (Aplica si no hay token o si el ID parece generado localmente)
    if (token == null || token.isEmpty || childId == '17' || childId.length > 10) {
      await Future.delayed(const Duration(milliseconds: 600));
      return 'DEMO${DateTime.now().millisecond.toString().padLeft(3, '0')}';
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          ...
          throw Exception('Error al regenerar código (${response.statusCode})');
        }
      }
    } catch (e) {
      if (e.toString().contains('Exception:')) {
