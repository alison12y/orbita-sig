...
class _ChildHomeScreenState extends ConsumerState<ChildHomeScreen> {
  ...
  Future<void> _cleanup() async {
    ...
    try {
      ...
      print('Error canceling position subscription: $e');
    }
    _positionSubscription = null;
    ...
    try {
      ...
      print('Error with socket service cleanup: $e');
    }
  ...
  Future<void> _updateLocationOnBackend(double lat, double lng) async {
    try {
      ...
      final baseUrl = dotenv.env['API_URL'] ?? 'https://backend-sig-l0wo.onrender.com';
  ...
  Future<void> _startLocationTracking() async {
    ...
    if (permission == LocationPermission.deniedForever) {
      setState(() => _lastLocationStatus = 'Permiso denegado. Actívalo en ajustes.');
      return;
    }

    try {
      if (await Permission.locationAlways.isDenied) {
        setState(() => _lastLocationStatus = 'Solicitando permiso de fondo...');
      ...
      print('Platform does not support background permission check: $e');
    }

    bool serviceEnabled = await Geolocator.isLocationServiceEnabled();
    if (!serviceEnabled) {
      setState(() => _lastLocationStatus = 'GPS desactivado. Enciéndelo.');
      // Continuamos de todas formas, el stream puede fallar pero no rompe la app
    }
    ...
    try {
      ...
      // Iniciar el rastreo inmediatamente independientemente del WebSocket,
      // para asegurar que las peticiones HTTP funcionen
      if (mounted) {
        setState(() {
          _isTracking = true;
          _lastLocationStatus = 'Buscando GPS...';
        });
      }
